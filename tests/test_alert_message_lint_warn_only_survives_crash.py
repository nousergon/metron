...
WORKFLOW = REPO / ".github" / "workflows" / "alert-message-lint.yml"
...
def test_pyyaml_is_installed_before_the_lint_runs():
    """metron's own root cause. Unlike the other 9 emitter repos, metron has
    no `alert-class-pr-guard` step (`alpha-engine-config-I9466`: metron does
    not publish onto the fleet alert bus, so the class-guard scanner would
    match nothing here) -- and it was that OTHER step's `Install guard
    dependencies` step that incidentally left `pyyaml` importable for this
    one everywhere else. Without a step of its own, `alert_message_lint.py`'s
    import chain (`alert_class_pr_guard` -> `alert_class_registry_drift` ->
    `yaml`) has nothing installed to import, and crashed exactly this way in
    run 33718023593."""
    text = WORKFLOW.read_text()
    install_idx = text.find("pip install")
    assert install_idx != -1 and "pyyaml" in text[install_idx:install_idx + 40], (
        f"{WORKFLOW.name} never installs pyyaml -- alert_message_lint.py "
        "imports it transitively and will crash with "
        "ModuleNotFoundError (alpha-engine-config-I10147)"
    )
    lint_idx = text.find(MARKER)
    assert install_idx < lint_idx, (
        f"{WORKFLOW.name} installs pyyaml AFTER the lint step runs -- too "
        "late to help it"
    )
