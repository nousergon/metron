...
# editing a file. This repo carried no coverage badge at all (the sweep treats
# that the same way a hand-set one is treated: Tier A must carry one, §5.3).
#
...
# from the same `.coverage` file the `--cov-fail-under=95` gate in
# `.github/workflows/ci.yml` exits non-zero on, and it moves on its own.
#
# Runs from .github/workflows/ci.yml on pushes to main only. The `badges`
# branch is seeded before the PR that adds this script (baseline/I10428), so
# the contents API is enough and no branch is ever created here.
set -euo pipefail
...
# `portfolio_analytics` + `api` source in pyproject.toml's [tool.coverage.run],
# not a figure re-derived from parsed stdout.
