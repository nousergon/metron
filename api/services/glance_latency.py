...
Three parts, one per deliverable:
...
def latest(session: Session) -> dict:
    """The most recently computed day's figure for ``/meta/status`` — ``status`` is
    ``"not-measured"`` (never a zero, never green) when no day has ever been recorded."""
    row = session.scalar(select(models.GlanceLatencyDaily).order_by(models.GlanceLatencyDaily.trading_day.desc()).limit(1))
    if row is None:
        return {"status": "not-measured", "trading_day": None, "p95_ms": None, "n": None, "target_ms": 1000.0}
    return {
        "status": "measured",
