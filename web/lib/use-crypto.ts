...
// SWR-backed crypto cache (metron-ops#232) — the page's SSR fetch seeds `fallbackData`
...
export function useCrypto(portfolioId: string, fallbackData: CryptoSummary) {
  return useSWR<CryptoSummary>(cryptoKey(portfolioId), () => fetchCryptoAction(portfolioId), {
    fallbackData,
